...
- YApi 运行页增强：在 URL 行下方常驻显示可编辑 cURL 面板，支持复制、粘贴编辑和回填参数。
- 路径参数引导：接口路径包含 `{param}` 时，点击发送会提示填写缺失参数。
- 请求历史：在 YApi 运行页显示最近请求记录，并支持从历史记录复制 cURL、回填请求参数。
...
- URL 行下方会常驻显示 cURL 面板，发送请求后会自动刷新为当前请求对应的 cURL；也可以手动粘贴 cURL 后点击「回填参数」。
- 请求会写入当前项目/接口维度的本地历史，点击「请求历史」可以查看最近记录、展开响应内容、复制历史 cURL，也可以把历史请求的 path/query/header/body 回填到当前运行页。
